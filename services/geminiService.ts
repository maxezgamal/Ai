...
export const generateTopicSuggestions = async (): Promise<string[]> => {
    const prompt = `
    أنت خبير استراتيجي في مجال التواصل الاجتماعي لعيادة "Dr. Hayam Neuro Clinic" في مصر، وهي عيادة متكاملة تقدم خدمات في أمراض المخ والأعصاب، بالإضافة إلى الصحة النفسية، العلاج الطبيعي، وإعادة التأهيل.

    مهمتك هي اقتراح 4 أفكار لمواضيع منشورات متنوعة وجذابة للمجتمع المصري، تغطي جميع تخصصات العيادة.
    
    ركز على استراتيجيات المحتوى التي تزيد من التفاعل والانتشار (Reach)، مثل:
    - **"نصيحة في دقيقة":** نصائح عملية وسريعة (للأعصاب، النفسية، أو الحركة).
    - **"حقيقة أم خرافة؟":** تصحيح المفاهيم الطبية الشائعة في كل التخصصات.
    - **"جسمك بيقولك إيه؟":** تفسير مبسط لأعراض شائعة (جسدية أو نفسية).
    - **"خطوة نحو التعافي":** تمارين بسيطة للعلاج الطبيعي أو نصائح للصحة النفسية.

    يجب أن تكون المواضيع المقترحة قصيرة، ومثيرة للفضول، ومكتوبة باللغة العربية.

    أمثلة على المواضيع الجيدة والمتنوعة:
    - **(مخ وأعصاب):** "لماذا تشعر بـ 'كهرباء' أو تنميل في أطرافك؟"
    - **(صحة نفسية):** "كيف تفرق بين الحزن الطبيعي وبدايات الاكتئاب؟"
    - **(علاج طبيعي):** "تمرين إطالة بسيط لآلام أسفل الظهر بسبب الجلوس الطويل."
    - **(إعادة تأهيل):** "أهمية الدعم النفسي للأسرة بعد إصابة أحد أفرادها بجلطة."
...
export const generateContent = async (topic: string): Promise<GeneratedContent> => {
  const prompt = `
    أنت نظام ذكاء اصطناعي متخصص في إنشاء محتوى يومي لمنصات التواصل الاجتماعي لـ "Dr. Hayam Neuro Clinic" — عيادة متكاملة في مصر تقدم خدمات في أمراض المخ والأعصاب، الصحة النفسية، العلاج الطبيعي، وإعادة التأهيل.
    
    مهمتك هي إنشاء محتوى متكامل (نص وصورة) بناءً على موضوع اليوم.
    موضوع اليوم هو: "${topic}"

    **أولاً: إعداد النص (شخصية طبيب مصري خبير ومتعدد التخصصات)**
    - اكتب بوستًا من 4 فقرات بلغة عربية بسيطة ودافئة بلهجة مصرية.
    1.  **فقرة أولى (Hook):** ابدأ بجملة قوية تمس مشكلة يعاني منها المصريون تتعلق بالموضوع (سواء كانت عصبية، نفسية، أو حركية).
    ...
    **ثانيًا: إعداد وصف الصورة (شخصية مصمم طبي محترف، فنان ومبدع)**
    ...
    - **تحديد الشخصيات:** بناءً على موضوع البوست "${topic}"، حدد بذكاء الفئة الأكثر تأثرًا به (شباب، كبار سن، رجال، نساء، أطفال). يجب أن تعكس الشخصيات في الصورة هذه الفئة العمرية والجنس بوضوح.
        - **مثال:** لو الموضوع عن الزهايمر، ركز على كبار السن. لو عن ضغط العمل، ركز على الشباب. لو عن تمرين علاج طبيعي، يمكن أن تظهر شخصًا من أي عمر يؤدي التمرين.
    ...
    - **الإبداع في المشهد:** لا تكتفِ بتصوير الشخص المريض. ابتكر مشاهد رمزية قوية.
        - **مثال عصبي:** لضعف التركيز، صورة شخص ملامحه تتلاشى وتتداخل مع صفحات كتاب.
        - **مثال نفسي:** للتعبير عن الدعم النفسي، صورة يدين تمسكان ببعضهما بحنان مع خلفية دافئة ومريحة.
        - **مثال حركي:** للأرق، غرفة نوم بسقف على شكل سماء ليلية مضطربة.
